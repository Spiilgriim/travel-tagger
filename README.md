Student project to create a system to tag geographically and activity-wise blog articles for [TravelAssist.io](https://www.travelassist.io/)

# Build Environnement

Use conda to setup a virtual environment

## Library List

```bash
# packages in environment at /home/alexandre/.anaconda3/envs/travelassist:
#
# Name                    Version                   Build  Channel
_libgcc_mutex             0.1                        main
asn1crypto                1.2.0                    py37_0    conda-forge
astroid                   2.3.2                    pypi_0    pypi
attrs                     19.3.0                     py_0
autopep8                  1.4.4                    pypi_0    pypi
backcall                  0.1.0                    py37_0
beautifulsoup4            4.8.0                    py37_0    anaconda
blas                      1.0                    openblas
bleach                    3.1.0                    py37_0
boto                      2.49.0                   py37_0    anaconda
boto3                     1.10.19                    py_0    anaconda
botocore                  1.13.19                    py_0    anaconda
bz2file                   0.98                     py37_1    anaconda
ca-certificates           2019.10.16                    0
certifi                   2019.9.11                py37_0
cffi                      1.13.0           py37h8022711_0    conda-forge
chardet                   3.0.4                 py37_1003
cryptography              2.7              py37h72c5cf5_0    conda-forge
cymem                     2.0.2            py37hfd86e86_0
cytoolz                   0.9.0.1          py37h14c3975_1
dbus                      1.13.6               h746ee38_0
decorator                 4.4.0                    py37_1
defusedxml                0.6.0                      py_0
dill                      0.2.9                    py37_0
docutils                  0.15.2                   py37_0    anaconda
entrypoints               0.3                      py37_0
expat                     2.2.6                he6710b0_0
fontconfig                2.13.0               h9420a91_0
fr-core-news-md           2.0.0                    pypi_0    pypi
fr-core-news-sm           2.0.0                    pypi_0    pypi
freetype                  2.9.1                h8a8886c_1
gensim                    3.8.0            py37h962f231_0    anaconda
glib                      2.56.2               hd408876_0
gmp                       6.1.2                h6c8ec71_1
gst-plugins-base          1.14.0               hbbd80ab_1
gstreamer                 1.14.0               hb453b48_1
html5lib                  1.0.1                    py37_0    anaconda
icu                       58.2                 h9c2bf20_1
idna                      2.8                   py37_1000    conda-forge
importlib_metadata        0.23                     py37_0
intel-openmp              2019.4                      243
ipykernel                 5.1.2            py37h39e3cac_0
ipython                   7.8.0            py37h39e3cac_0
ipython_genutils          0.2.0                    py37_0
ipywidgets                7.5.1                      py_0
isort                     4.3.21                   pypi_0    pypi
jedi                      0.15.1                   py37_0
jinja2                    2.10.3                     py_0
jmespath                  0.9.4                      py_0    anaconda
joblib                    0.13.2                   py37_0
jpeg                      9b                   h024ee3a_2
jsonschema                3.1.1                    py37_0
jupyter                   1.0.0                    py37_7
jupyter_client            5.3.4                    py37_0
jupyter_console           6.0.0                    py37_0
jupyter_core              4.6.0                    py37_0
lazy-object-proxy         1.4.2                    pypi_0    pypi
libedit                   3.1.20181209         hc058e9b_0
libffi                    3.2.1                hd88cf55_4
libgcc-ng                 9.1.0                hdf63c60_0
libgfortran-ng            7.3.0                hdf63c60_0
libopenblas               0.3.6                h5a2b251_1
libpng                    1.6.37               hbc83047_0
libsodium                 1.0.16               h1bed415_0
libstdcxx-ng              9.1.0                hdf63c60_0
libuuid                   1.0.3                h1bed415_2
libxcb                    1.13                 h1bed415_1
libxml2                   2.9.9                hea5a465_1
libxslt                   1.1.33               h7d1a2b0_0    anaconda
lxml                      4.4.1            py37hefd8a0e_0    anaconda
markupsafe                1.1.1            py37h7b6447c_0
mccabe                    0.6.1                    pypi_0    pypi
mistune                   0.8.4            py37h7b6447c_0
mkl                       2019.4                      243
mkl-service               2.3.0            py37he904b0f_0
more-itertools            7.2.0                    py37_0
msgpack-numpy             0.4.3.2                  py37_0
msgpack-python            0.6.1            py37hfd86e86_1
murmurhash                1.0.2            py37he6710b0_0
nbconvert                 5.6.0                    py37_1
nbformat                  4.4.0                    py37_0
ncurses                   6.1                  he6710b0_1
notebook                  6.0.1                    py37_0
numpy                     1.17.2           py37hf9bf1b6_0
numpy-base                1.17.2           py37h2f8d375_0
openssl                   1.1.1d               h7b6447c_3
pandas                    0.25.2           py37he6710b0_0
pandoc                    2.2.3.2                       0
pandocfilters             1.4.2                    py37_1
parso                     0.5.1                      py_0
pcre                      8.43                 he6710b0_0
pexpect                   4.7.0                    py37_0
pickleshare               0.7.5                    py37_0
pip                       19.3.1                   py37_0
plac                      0.9.6                    py37_0
preshed                   2.0.1            py37he6710b0_0
prometheus_client         0.7.1                      py_0
prompt_toolkit            2.0.10                     py_0
ptyprocess                0.6.0                    py37_0
pycodestyle               2.5.0                    pypi_0    pypi
pycparser                 2.19                     py37_1    conda-forge
pygments                  2.4.2                      py_0
pylint                    2.4.3                    pypi_0    pypi
pyopenssl                 19.0.0                   py37_0    conda-forge
pyqt                      5.9.2            py37h05f1152_2
pyrsistent                0.15.4           py37h7b6447c_0
pysocks                   1.7.1                    py37_0    conda-forge
python                    3.7.4                h265db76_1
python-dateutil           2.8.0                    py37_0
pytz                      2019.3                     py_0
pyzmq                     18.1.0           py37he6710b0_0
qt                        5.9.7                h5867ecd_1
qtconsole                 4.5.5                      py_0
readline                  7.0                  h7b6447c_5
regex                     2019.08.19       py37h7b6447c_0
requests                  2.22.0                   py37_0
s3transfer                0.2.1                    py37_0    anaconda
scikit-learn              0.21.3           py37h22eb022_0
scipy                     1.3.1            py37he2b7bc3_0
send2trash                1.5.0                    py37_0
setuptools                41.4.0                   py37_0
sip                       4.19.8           py37hf484d3e_0
six                       1.12.0                   py37_0
smart_open                1.9.0                      py_0    anaconda
soupsieve                 1.9.3                    py37_0    anaconda
spacy                     2.0.16           py37h962f231_0
sqlite                    3.30.0               h7b6447c_0
terminado                 0.8.2                    py37_0
testpath                  0.4.2                    py37_0
thinc                     6.12.1           py37h22eb022_0
tk                        8.6.8                hbc83047_0
toolz                     0.10.0                     py_0
tornado                   6.0.3            py37h7b6447c_0
tqdm                      4.39.0                     py_0
traitlets                 4.3.3                    py37_0
typed-ast                 1.4.0                    pypi_0    pypi
ujson                     1.35             py37h14c3975_0
urllib3                   1.24.2                   py37_0
wcwidth                   0.1.7                    py37_0
webencodings              0.5.1                    py37_1
wheel                     0.33.6                   py37_0
widgetsnbextension        3.5.1                    py37_0
wikipedia                 1.4.0                      py_2    conda-forge
wrapt                     1.11.2                   pypi_0    pypi
xx-ent-wiki-sm            2.0.0                    pypi_0    pypi
xz                        5.2.4                h14c3975_4
zeromq                    4.3.1                he6710b0_3
zipp                      0.6.0                      py_0
zlib                      1.2.11               h7b6447c_3
```

For more details checkout [conda's doc](https://docs.conda.io/projects/conda/en/latest/index.html)

# Usage

You can try these programs after setting up your environment by adjusting the configuration file to your liking and running the different functionalities you need:

## Configuration

- `ban_list`: Words that if contained in a url will make the url ignored when fetching the content of a website
- `folder_path`: Folder containing the articles to use
- `target_path`: Path to save the data

## Run Options

- `-h, --help`: displays help
- `--fetch_website url`: fetch the content of a website by web crawling, will store it in the articles folder with automatically generated name
- `--location_tag folder_path`: find location tags of articles in the folder article_path will by default use the whole articles folder as path
- `--clustering n_clusters`: cluster articles (using KMeans algorithm) with articles TD-IDF as feature will by default use the whole articles folder as path (modify in the config file)
- `--topic_modeling model_name`: perform topic modeling with articles TD-IDF as feature will by default use the whole articles folder as path (modify in the config file)
- `--label_data data_number`: Label data number_article selected randomly from the folder article_path (modify in the config file) write it at labeled_data.csv (modify target path in the config file)
- `--naive_bayes labeled_csv`: perform naive bayes classification on data expect csv path to labeled data
- `--svm_classifier labeled_csv`: perform naive bayes classification on data expect csv path to labeled data
- `--random_forest labeled_csv`: perform naive bayes classification on data expect csv path to labeled data"

```bash
conda activate myenv

python3 main.py [--fetchwebsite][url][--location_tag][folder_path][--clustering][n_clusters][--topic_modeling][model_name][--label_data][data_number][--naive_bayes][labeled_csv][--svm_classifier][labeled_csv][--random_forest][labeled_csv][-h]

conda deactivate
```

PR and Bug report are appreciated
